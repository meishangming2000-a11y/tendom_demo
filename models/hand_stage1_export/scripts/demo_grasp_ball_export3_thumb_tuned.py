...
REPORT_MD = DOCS_DIR / "export3_grasp_ball_thumb_tuned_report.md"
REPORT_JSON = METADATA_DIR / "export3_grasp_ball_thumb_tuned.json"
OUT_DIR = DOCS_DIR / "visual_checks_export3_thumb_tuned_grasp"
THUMB_AUDIT_JSON = METADATA_DIR / "export3_thumb_limit_tuning_candidates.json"


def load_thumb_targets() -> tuple[dict[str, float], str]:
    if not THUMB_AUDIT_JSON.exists():
        return dict(THUMB_CLOSE_TARGETS), "fallback"
    try:
        payload = json.loads(THUMB_AUDIT_JSON.read_text(encoding="utf-8"))
        best = payload.get("recommended_scripted_pose") or payload.get("best_by_score", [])[0]["applied_pose"]
        return {name: float(best.get(name, 0.0)) for name in THUMB_JOINTS}, str(THUMB_AUDIT_JSON)
    except Exception as exc:
        return dict(THUMB_CLOSE_TARGETS), f"fallback_after_error:{type(exc).__name__}:{exc}"
...
def interpret(report: dict) -> list[str]:
    lines = []
    open_stage = report["stage_reports"].get("open_hand", {})
    hold = report["stage_reports"].get("hold", {})
    if open_stage.get("ball_contact_count", 0) == 0:
        lines.append("Open hand has no initial ball contact at the default tuned ball position.")
    else:
        lines.append("Open hand starts with contact; ball placement should be moved before free-object tests.")
    lines.append(f"Hold thumb-ball distance is {hold.get('thumb_to_ball', 0.0):.4f} m.")
    lines.append(f"Hold thumb-index distance is {hold.get('thumb_to_index', 0.0):.4f} m.")
    lines.append(f"Hold mean four-fingertip distance is {hold.get('mean_four_tip_distance', 0.0):.4f} m.")
    if hold.get("thumb_to_ball", 1.0) < 0.06:
        lines.append("Tuned thumb pose enters the 0.06 m thumb-ball target region.")
    if hold.get("max_penetration", 0.0) > 0.012:
        lines.append("Hold penetration is noticeable; this remains a pinned-ball smoke test, not physical grasp validation.")
    lines.append("The tuned thumb target is suitable for scripted Shadow-style scaffold experiments, but not yet for training promotion.")
    return lines
...
def write_outputs(report: dict) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Thumb target: `{report.get('thumb_close_target')}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export3 thumb-tuned position-control ball grasp smoke test.")
    ...
    thumb_targets, thumb_source = load_thumb_targets()
    ...
    for stage in STAGE_ORDER:
        ...
        if not args.no_screenshots:
            ...
            if stage in {"close_thumb", "hold"}:
                renders.append(render_png(model, data, mujoco, "top", OUT_DIR / f"{stage}_top.png", width=1200, height=850))
                renders.append(render_png(model, data, mujoco, "palm", OUT_DIR / f"{stage}_palm.png", width=1200, height=850))
        previous = targets

    if viewer is not None:
        print("Export3 thumb-tuned demo reached hold stage. Close the viewer window to exit.")
