#!/usr/bin/env python3
"""Apply the v5 whole-arm mount feedback on top of the v3 body scene.

V4 incorrectly interpreted the user's "hand/arm needs another horizontal
90 deg turn" as a local hand twist. V5 applies the correction to the whole
arm chain at `base_link` so the arm aligns with the body mount frame.
"""
...
OUT_SCENE = ROOT / "mjcf" / "scene_export4_connected_to_body_corrected_v5.xml"
DOCS = ROOT / "docs"
VIS = DOCS / "visual_checks_body_connection_corrected_v5"
CANDIDATE_DIR = VIS / "candidates"
REPORT = DOCS / "body_connection_orientation_corrected_v5_report.md"
META = ROOT / "metadata" / "body_connection_orientation_corrected_v5.json"

BODY_ROOT_NAME = "rough_body_support_link"
ARM_ROOT_NAME = "base_link"
HAND_ROOT_NAME = "hand_base_link"
SUPPORT_COLUMN_NAME = "body_support_column_visual_v5"

BODY_ROOT_Z_LIFT_M = 0.18
ARM_WORLD_YAW_DEG = 90.0
V3_ARM_QUAT_WXYZ = np.array([4.329780281e-17, 0.7071067812, 4.329780281e-17, 0.7071067812], dtype=np.float64)
...
def apply_v5_delta(tree: ET.ElementTree) -> dict[str, Any]:
    root = tree.getroot()
    root.set("model", "export4_connected_to_body_corrected_v5")
    ...
    old_arm_quat = np.fromstring(arm_root.get("quat", "1 0 0 0"), sep=" ")
    yaw = axis_angle((0, 0, 1), ARM_WORLD_YAW_DEG)
    new_arm_quat = quat_mul(yaw, old_arm_quat)
    arm_root.set("quat", fmt_vec(new_arm_quat))

    hand_root = find_required(root, f".//body[@name='{HAND_ROOT_NAME}']", label=HAND_ROOT_NAME)
    hand_quat = np.fromstring(hand_root.get("quat", "1 0 0 0"), sep=" ")
    ...
    return {
        ...
        "arm_world_yaw_deg": ARM_WORLD_YAW_DEG,
        "new_arm_quat_wxyz": new_arm_quat,
...
def pose_metrics(model: mujoco.MjModel, data: mujoco.MjData) -> dict[str, Any]:
    ...
    base = positions.get(ARM_ROOT_NAME)
...
def inspect_scene() -> dict[str, Any]:
    ...
    shots = {
        "front": render(model, data, CANDIDATE_DIR / "selected_v5_front.png", center, distance=1.18, azimuth=205, elevation=-20),
        "side": render(model, data, CANDIDATE_DIR / "selected_v5_side.png", center, distance=1.18, azimuth=110, elevation=-16),
        "mount": render(model, data, CANDIDATE_DIR / "selected_v5_mount.png", arm_center, distance=0.72, azimuth=205, elevation=-14),
    }
    return {
        "model_summary": {
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "status": "v5_whole_arm_alignment_candidate_needs_user_confirmation",
        "supersedes": {
            "scene": "simulations/models/hand_stage1_export/mjcf/scene_export4_connected_to_body_corrected_v4.xml",
            "reason": "v4 rotated only hand_base_link; user clarified the whole arm must rotate to align with body.",
        },
...
def write_outputs(payload: dict[str, Any]) -> None:
    ...
    delta = payload["v5_delta"]
    metrics = payload["inspection"]["metrics"]
    lines = [
        "# Body Connection Orientation Corrected V5 Report\n\n",
        f"Generated: {payload['generated_at']}\n\n",
        f"- Source scene: `{payload['source_scene']}`\n",
        f"- Corrected scene: `{payload['output_scene']}`\n",
        f"- Status: `{payload['status']}`\n",
        f"- Supersedes v4 reason: `{payload['supersedes']['reason']}`\n",
        ...
        f"- New arm quat wxyz: `{fmt_vec(delta['new_arm_quat_wxyz'])}`\n",
        ...
        f"- base_to_palm: `{fmt_vec(metrics['base_to_palm'])}`\n",
    ...
    lines.extend(
        [
            ...
            "## Notes\n\n",
            "- V5 rotates the entire arm chain at `base_link`; it does not rotate the palm or hand independently.\n",
