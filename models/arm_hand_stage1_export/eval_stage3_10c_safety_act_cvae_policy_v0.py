#!/usr/bin/env python3
"""Closed-loop MuJoCo eval for the Stage3.10C safety ACT/CVAE chunk policy.

This evaluator is intentionally conservative. It reuses the Stage3 ACT-lite
closed-loop harness, but loads the Stage3.10C SafetyACTCVAEChunkPolicy and feeds
the extra phase/safety context that was used during offline training.
...
The second mode is the first honest gate for this model: it tests whether the
learned hand/finger chunk can survive closed-loop MuJoCo feedback before trying
full 26-actuator policy execution.
"""
...
from train_stage3_10d_c_occlusion_safety_head_v0 import (
    FEATURE_NAMES as SAFETY_HEAD_FEATURE_NAMES,
    LABEL_NAMES as SAFETY_HEAD_LABEL_NAMES,
    make_features_and_labels as make_safety_head_features_and_labels,
    predict as predict_safety_head_labels,
)
from train_stage3_10c_safety_act_cvae_policy_v0 import SafetyACTCVAEChunkPolicy
...
from run_stage3_noisy_virtual_camera_perception_test import NoiseScenario, estimate_noisy_pose, noise_scenarios, scenario_rng
...
DEFAULT_SAFETY_HEAD = CHECKPOINTS / "stage3_10d_c_occlusion_safety_head_v0.npz"
DEFAULT_REPORT = DOCS / "stage3_10c_safety_act_cvae_policy_v0_closed_loop_eval_report.md"
DEFAULT_METADATA = META / "stage3_10c_safety_act_cvae_policy_v0_closed_loop_eval.json"
...
def vision_stress_scenario(name: str | None) -> NoiseScenario | None:
    raw = str(name or "none").strip()
    if raw.lower() in {"", "none", "off", "clean", "clean_reference"}:
        return None
    table = {scenario.name: scenario for scenario in noise_scenarios()}
    table.update(
        {
            "occluder_60": NoiseScenario("occluder_60", occluder_fraction=0.60),
            "mask_dropout_70": NoiseScenario("mask_dropout_70", mask_dropout=0.70),
            "combined_occluder_dropout": NoiseScenario(
                "combined_occluder_dropout",
                mask_dropout=0.45,
                occluder_fraction=0.45,
                depth_noise_std_m=0.004,
            ),
        }
    )
    if raw not in table:
        raise ValueError(f"Unknown vision stress scenario {raw!r}; choose from none or {sorted(table)}")
    return table[raw]


def acquire_vision_with_optional_stress(
    *,
    model: Any,
    data: Any,
    mujoco: Any,
    ...
    episode_id: int,
    label: str,
    stress_samples: int = 1,
) -> dict[str, Any]:
    scenario = vision_stress_scenario(scenario_name)
    if scenario is None:
        sensor = MujocoEggPoseSensor(
            model,
            mujoco,
            EggPoseSensorConfig(camera_name=camera_name, width=int(width), height=int(height), min_mask_pixels=int(min_mask_pixels)),
        )
        estimate = sensor.estimate(data, label=label)
        estimate["vision_stress_scenario"] = "none"
        estimate["vision_stress_sample_index"] = 0
        estimate["vision_stress_samples"] = 1
    else:
        estimates: list[dict[str, Any]] = []
        sample_count = max(1, int(stress_samples))
        ...
        for sample_idx in range(sample_count):
            rng = scenario_rng(
                int(seed) + sample_idx * 7919,
                f"{trial_name}:{int(episode_id)}:sample{sample_idx}",
                f"{label}:{camera_name}:{scenario.name}",
            )
            item = estimate_noisy_pose(
                model=model,
                data=data,
                mujoco=mujoco,
                config=config,
                ...
                rng=rng,
                debug_dir=None,
                label=f"{label}_s{sample_idx}",
            )
            item["vision_stress_scenario"] = scenario.name
            item["vision_stress_sample_index"] = int(sample_idx)
            item["vision_stress_samples"] = int(sample_count)
            item["camera"] = camera_name
            item["accepted"] = bool(item.get("status") == "ok" and float(item.get("confidence", 0.0)) >= float(min_confidence))
            if item["accepted"]:
                item.setdefault("accepted_by", "strict_vision")
            estimates.append(item)
        accepted = [item for item in estimates if bool(item.get("accepted"))]
        ranked = accepted if accepted else estimates
        ranked.sort(key=lambda item: (float(item.get("confidence", 0.0)), int(item.get("mask_pixels", 0))), reverse=True)
        estimate = ranked[0]
        estimate["vision_stress_candidate_count"] = int(len(estimates))
        estimate["vision_stress_best_confidence"] = float(estimate.get("confidence", 0.0))
        estimate["vision_stress_best_mask_pixels"] = int(estimate.get("mask_pixels", 0))
    estimate["camera"] = camera_name
    estimate["accepted"] = bool(estimate.get("status") == "ok" and float(estimate.get("confidence", 0.0)) >= float(min_confidence))
    if estimate["accepted"]:
        estimate.setdefault("accepted_by", "strict_vision")
    return estimate


def best_final_vision_with_optional_stress(
    *,
    model: Any,
    data: Any,
    mujoco: Any,
    ...
    estimates = [
        acquire_vision_with_optional_stress(
            model=model,
            data=data,
            mujoco=mujoco,
            camera_name=camera,
            ...
            label=f"{label}_{camera}",
            stress_samples=int(stress_samples),
        )
...
def policy_vision_fields(vision_estimate: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    vision = collect.make_vision_fields(vision_estimate)
    mode = str(getattr(args, "policy_vision_quality_mode", "raw"))
    if mode == "accepted_nominal" and bool(vision_estimate.get("accepted")):
        vision = dict(vision)
        vision["confidence"] = max(
            float(vision.get("confidence", 0.0)),
            float(getattr(args, "policy_vision_nominal_min_confidence", 0.95)),
        )
        vision["occlusion"] = min(
            float(vision.get("occlusion", 1.0)),
            float(getattr(args, "policy_vision_nominal_max_occlusion", 0.05)),
        )
        vision["noise_std"] = min(
            float(vision.get("noise_std", 0.0)),
            float(getattr(args, "policy_vision_nominal_max_noise_std", 0.0)),
        )
    return vision


def load_chunk_policy(path: Path, device: torch.device) -> tuple[SafetyACTCVAEChunkPolicy, dict[str, Any]]:
    ...
    model = SafetyACTCVAEChunkPolicy(
        ...
        phase_count=int(checkpoint["phase_count"]),
        safety_dim=int(checkpoint["safety_dim"]),
...
def phase_index_from_checkpoint(checkpoint: dict[str, Any], phase_name: str) -> int:
    phase_table = [str(item) for item in checkpoint.get("phase_table", [])]
    mapped = collect.STAGE3_PHASE_MAP.get(str(phase_name), str(phase_name))
    if mapped in phase_table:
        return int(phase_table.index(mapped))
    if str(phase_name) in phase_table:
        return int(phase_table.index(str(phase_name)))
    return int(phase_table.index("approach")) if "approach" in phase_table else 0


def current_safety_labels(
    ...
    *,
    ...
    floor_contact = float(tactile.get("floor_contact", 0.0))
    ...
    risk_floor_contact = floor_contact > 0.0
    risk_any = risk_slip_high or risk_crush_high or risk_penetration_high or risk_floor_contact
    safety_active = bool(recovery_active or repair_active or risk_any)
    return np.asarray(
        [
            bool(learned_hand),
...
def predict_chunk(
    policy: SafetyACTCVAEChunkPolicy,
    ...
    with torch.no_grad():
        ...
        phase_t = torch.tensor([phase_index_from_checkpoint(checkpoint, phase_name)], dtype=torch.long, device=device)
        safety_t = torch.from_numpy(np.asarray(safety_now, dtype=np.float32)[None, :]).to(device)
        pred_actions, pred_safety = policy.predict(obs_t, skill_t, phase_t, safety_t)
    chunk = pred_actions.detach().cpu().numpy()[0].reshape(int(checkpoint["horizon"]), int(checkpoint["action_dim"]))
    safety_chunk = pred_safety.detach().cpu().numpy()[0].reshape(int(checkpoint["horizon"]), int(checkpoint["safety_dim"]))
    return chunk * action_std.reshape(1, -1) + action_mean.reshape(1, -1), safety_chunk


def load_safety_head_shadow(path: Path) -> dict[str, Any]:
...
def safety_head_shadow_case(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "case_id": str(getattr(args, "safety_head_shadow_case_id", "online_shadow")),
        "random_offset_std": float(getattr(args, "random_offset_std", 0.0)),
        "final_verification_mode": str(getattr(args, "pinch_final_verification_mode", "strict_vision")),
        "initial_vision_stress_scenario": str(getattr(args, "initial_vision_stress_scenario", "none")),
        "final_vision_stress_scenario": str(getattr(args, "final_vision_stress_scenario", "none")),
        "initial_vision_stress_samples": int(getattr(args, "initial_vision_stress_samples", 1)),
        "final_vision_stress_samples": int(getattr(args, "final_vision_stress_samples", 1)),
    }


def attach_safety_head_shadow(
    row: dict[str, Any],
    *,
    ...
    features, labels, feature_row = make_safety_head_features_and_labels(
        benchmark_path=Path(args.metadata),
        case=safety_head_shadow_case(args),
        payload={"args": vars(args)},
        result=row,
    )
    feature_arr = np.asarray(features, dtype=np.float32).reshape(1, -1)
    normalized = (feature_arr - head["feature_mean"].reshape(1, -1)) / head["feature_std"].reshape(1, -1)
    probabilities = predict_safety_head_labels(head["weights"], normalized)[0]
    predictions = probabilities >= float(threshold)
    truth = np.asarray(labels, dtype=np.float32) >= 0.5
    label_idx = {name: idx for idx, name in enumerate(SAFETY_HEAD_LABEL_NAMES)}
    mismatch_labels = [
        name
        for idx, name in enumerate(SAFETY_HEAD_LABEL_NAMES)
        if bool(predictions[idx]) != bool(truth[idx])
    ]
    predicted_labels = {
        name: bool(predictions[idx])
        for idx, name in enumerate(SAFETY_HEAD_LABEL_NAMES)
    }
    true_labels = {
        name: bool(truth[idx])
    ...
    row["safety_head_shadow"] = {
        "mode": "shadow_only_no_action_effect",
        "checkpoint": str(head["checkpoint"]),
        "threshold": float(threshold),
        "feature_names": list(SAFETY_HEAD_FEATURE_NAMES),
        "label_names": list(SAFETY_HEAD_LABEL_NAMES),
        "probabilities": {
        ...
        "false_failure_on_hold_safe": bool(
            true_labels.get("hold_safe", False)
            and predicted_labels.get("failure_or_blocked", False)
        ),
        "missed_failure_or_blocked": bool(
            true_labels.get("failure_or_blocked", False)
            and not predicted_labels.get("failure_or_blocked", False)
        ),
        "unsafe_predicted_hold_safe": bool(
            true_labels.get("failure_or_blocked", False)
            and predicted_labels.get("hold_safe", False)
        ),
        "unsafe_hold_safe_prediction": bool(
            true_labels.get("failure_or_blocked", False)
            and predicted_labels.get("hold_safe", False)
        ),
        "missed_tactile_handoff": bool(
            true_labels.get("final_tactile_handoff_required", False)
            and not predicted_labels.get("final_tactile_handoff_required", False)
        ),
        "feature_values": feature_row.get("feature_values", {}),
    }


def summarize_safety_head_shadow(rows: list[dict[str, Any]]) -> dict[str, Any]:
    shadows = [row.get("safety_head_shadow") for row in rows if isinstance(row.get("safety_head_shadow"), dict)]
    if not shadows:
        return {"enabled": False, "records": 0}
    per_label: dict[str, dict[str, int]] = {
        name: {"truth_positive": 0, "predicted_positive": 0, "mismatch": 0}
        for name in SAFETY_HEAD_LABEL_NAMES
    }
    for shadow in shadows:
        truth = shadow.get("true_labels", {})
        pred = shadow.get("predicted_labels", {})
        for name in SAFETY_HEAD_LABEL_NAMES:
            per_label[name]["truth_positive"] += int(bool(truth.get(name, False)))
            per_label[name]["predicted_positive"] += int(bool(pred.get(name, False)))
            per_label[name]["mismatch"] += int(bool(truth.get(name, False)) != bool(pred.get(name, False)))
    return {
        "enabled": True,
        "mode": "shadow_only_no_action_effect",
        "records": int(len(shadows)),
        "false_failure_on_hold_safe": int(sum(bool(item.get("false_failure_on_hold_safe")) for item in shadows)),
        "missed_failure_or_blocked": int(sum(bool(item.get("missed_failure_or_blocked")) for item in shadows)),
        "unsafe_predicted_hold_safe": int(sum(bool(item.get("unsafe_predicted_hold_safe")) for item in shadows)),
        "unsafe_hold_safe_prediction": int(sum(bool(item.get("unsafe_hold_safe_prediction")) for item in shadows)),
        "missed_tactile_handoff": int(sum(bool(item.get("missed_tactile_handoff")) for item in shadows)),
        "total_label_mismatches": int(sum(len(item.get("mismatch_labels", [])) for item in shadows)),
        "per_label": per_label,
    }
...
def make_pinch_args(args: argparse.Namespace) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        debug_dir=DOCS / "visual_checks_stage3_10c_safety_act_cvae_eval_v0",
    )
...
def action_from_policy(
    *,
    ...
    if chunk_cache.get("chunk") is None or int(chunk_cache.get("cursor", 0)) >= int(replan_interval):
        learned_hand = str(execution_mode) in {"full_action", "scripted_arm_predicted_hand"}
        safety_now = current_safety_labels(tactile, learned_hand=learned_hand)
        chunk, safety_chunk = predict_chunk(policy, checkpoint, obs, skill_index, phase_name, safety_now, device)
        chunk_cache["chunk"] = chunk
        chunk_cache["predicted_safety_chunk"] = safety_chunk
        chunk_cache["phase_name"] = str(phase_name)
        chunk_cache["safety_now"] = safety_now
        chunk_cache["cursor"] = 0
    idx = min(int(chunk_cache["cursor"]), horizon - 1)
    predicted = np.asarray(chunk_cache["chunk"][idx], dtype=np.float64)
    default_safety = np.zeros((horizon, int(checkpoint["safety_dim"])), dtype=np.float32)
    safety_pred = np.asarray(chunk_cache.get("predicted_safety_chunk", default_safety)[idx], dtype=np.float64)
    chunk_cache["last_predicted_safety"] = safety_pred
...
def run_full_hand_episode(
    *,
    ...
    vision_estimate = acquire_vision_with_optional_stress(
        model=model,
        data=data,
        mujoco=mujoco,
        ...
        min_mask_pixels=int(getattr(args, "vision_stress_min_mask_pixels", 80)),
        scenario_name=str(getattr(args, "initial_vision_stress_scenario", "none")),
        seed=int(cfg.seed),
        trial_name=trial.name,
        episode_id=int(episode_id),
        label=f"stage3_10c_full_{episode_id:04d}_vision_acquire",
        stress_samples=int(getattr(args, "initial_vision_stress_samples", 1)),
    )
    ...
    vision = collect.make_vision_fields(vision_estimate)
    profile = collect.Stage3DatasetProfile("stage3_10c_eval", 1.0, 0.0, 0.0, 0)
    ...
    summary = {
        ...
        "initial_vision_stress_scenario": vision_estimate.get("vision_stress_scenario", "none"),
        "initial_vision_stress_samples": int(vision_estimate.get("vision_stress_samples", 1)),
        "initial_vision_stress_sample_index": int(vision_estimate.get("vision_stress_sample_index", 0)),
        "initial_vision_confidence": float(vision_estimate.get("confidence", 0.0)),
        "initial_vision_mask_pixels": int(vision_estimate.get("mask_pixels", 0)),
        "initial_vision_mask_retention_ratio": float(vision_estimate.get("mask_retention_ratio", 1.0)),
        "initial_vision_visibility_fraction": float(vision_estimate.get("visibility_fraction", 0.0)),
        "policy_vision_quality_mode": str(getattr(args, "policy_vision_quality_mode", "raw")),
        "policy_vision_confidence": float(vision.get("confidence", 0.0)),
        "policy_vision_occlusion": float(vision.get("occlusion", 0.0)),
...
def run_full_hand_fallback_episode(
    *,
    ...
    row["policy_scope"] = "stage3_7d_full_hand_fallback_inside_stage3_10c_eval"
...
def run_pinch_episode(
    *,
    ...
    if float(cfg.random_offset_std) > 0.0:
        ...
        egg_position[2] = base_egg_position[2] + np.asarray(trial.offset_xyz, dtype=np.float64)[2]
    sweep.reset_episode(model, data, mujoco, egg_position)
    initial_vision = acquire_vision_with_optional_stress(
        model=model,
        data=data,
        mujoco=mujoco,
        camera_name=str(cfg.camera),
        ...
        trial_name=trial.name,
        episode_id=int(episode_id),
        label=f"stage3_10c_pinch_{episode_id:04d}_initial",
        stress_samples=int(getattr(args, "initial_vision_stress_samples", 1)),
    )
    if not bool(initial_vision.get("accepted")):
        initial_failure_summary = {
            "initial_vision_accepted": False,
            "initial_vision_stress_scenario": initial_vision.get("vision_stress_scenario", "none"),
            "initial_vision_stress_samples": int(initial_vision.get("vision_stress_samples", 1)),
            "initial_vision_stress_sample_index": int(initial_vision.get("vision_stress_sample_index", 0)),
            "initial_vision_status": initial_vision.get("status"),
            "initial_vision_failure_reason": initial_vision.get("reason", initial_vision.get("status", "unknown")),
            ...
            "initial_vision_mask_retention_ratio": float(initial_vision.get("mask_retention_ratio", 0.0)),
            "initial_vision_visibility_fraction": float(initial_vision.get("visibility_fraction", 0.0)),
            "policy_vision_quality_mode": str(getattr(args, "policy_vision_quality_mode", "raw")),
        }
        return {
            "episode_id": int(episode_id),
            ...
            "risk_flags": [],
            "summary": initial_failure_summary,
    ...
    vision = policy_vision_fields(initial_vision, args)
    thresholds = Stage3Thresholds()
    tactile_sensor = MujocoTactileSlipSensor(model, mujoco, thresholds=thresholds)
    tactile_sensor.reset(data)
    ...
    if capture_sequence:
        sequence_rows = {
            ...
            "predicted_safety_labels": [],
            "predicted_safety_active": [],
            "predicted_risk_any": [],
            "lift_heights": [],
            "pinch_contact": [],
    ...
    for phase_name, start, end, steps in plan["phases"]:
        ...
        while local_step < phase_steps:
            ...
            if sequence_rows is not None:
                ...
                predicted_safety = np.asarray(chunk_cache.get("last_predicted_safety", np.zeros(int(checkpoint["safety_dim"]), dtype=np.float32)), dtype=np.float32)
                sequence_rows["predicted_safety_labels"].append(predicted_safety)
                sequence_rows["predicted_safety_active"].append(bool(predicted_safety[3] >= 0.5) if predicted_safety.size > 3 else False)
                sequence_rows["predicted_risk_any"].append(bool(predicted_safety[8] >= 0.5) if predicted_safety.size > 8 else False)
    ...
    final_vision, final_vision_all = best_final_vision_with_optional_stress(
        model=model,
        data=data,
        mujoco=mujoco,
        ...
        min_mask_pixels=int(getattr(args, "vision_stress_min_mask_pixels", 80)),
        scenario_name=str(getattr(args, "final_vision_stress_scenario", "none")),
        seed=int(cfg.seed) + 17000,
        trial_name=trial.name,
        episode_id=int(episode_id),
        label=f"stage3_10c_pinch_{episode_id:04d}_final",
        stress_samples=int(getattr(args, "final_vision_stress_samples", 1)),
    )
    ...
    final_vision_occlusion_aware = False
    final_verification_mode = str(getattr(args, "pinch_final_verification_mode", "strict_vision"))
    tactile_hold_ok = (
        hold_steps > 0
    ...
    low_confidence_vision_ok = (
        final_vision.get("status") == "ok"
        and np.isfinite(final_vision_pos).all()
        and float(final_vision.get("confidence", 0.0)) >= float(args.pinch_final_corroboration_min_confidence)
        and int(final_vision.get("mask_pixels", 0)) >= int(args.pinch_final_corroboration_min_mask_pixels)
    )
    if not final_vision_primary_accepted and final_verification_mode == "vision_tactile_corroborated":
    ...
    elif not final_vision_primary_accepted and final_verification_mode == "vision_tactile_occlusion_aware":
        occluded_vision_present = bool(
            final_vision.get("status") == "ok"
            and int(final_vision.get("mask_pixels", 0)) >= int(getattr(args, "pinch_final_occlusion_aware_min_mask_pixels", 120))
        )
        final_vision_occlusion_aware = bool(occluded_vision_present and tactile_hold_ok)
        if final_vision_occlusion_aware:
            final_vision = dict(final_vision)
            final_vision["accepted"] = True
            final_vision["accepted_by"] = "vision_tactile_occlusion_aware"
            final_vision["primary_accepted"] = False
    final_vision_accepted = bool(final_vision.get("accepted"))
    if final_vision_accepted and not final_vision_occlusion_aware:
        vision_lift = float(final_vision_pos[2] - initial_vision_pos[2])
    else:
        vision_lift = float("nan")
    hold_stable_fraction = float(hold_stable_steps / max(1, hold_steps))
    ...
    summary = {
        ...
        "initial_vision_confidence": float(initial_vision.get("confidence", 0.0)),
        "initial_vision_mask_pixels": int(initial_vision.get("mask_pixels", 0)),
        "initial_vision_mask_retention_ratio": float(initial_vision.get("mask_retention_ratio", 1.0)),
        "initial_vision_visibility_fraction": float(initial_vision.get("visibility_fraction", 0.0)),
        "policy_vision_quality_mode": str(getattr(args, "policy_vision_quality_mode", "raw")),
        "policy_vision_confidence": float(vision.get("confidence", 0.0)),
        "policy_vision_occlusion": float(vision.get("occlusion", 0.0)),
        "final_vision_accepted": bool(final_vision_accepted),
        "final_vision_stress_scenario": final_vision.get("vision_stress_scenario", "none"),
        "final_vision_stress_samples": int(final_vision.get("vision_stress_samples", 1)),
        "final_vision_stress_sample_index": int(final_vision.get("vision_stress_sample_index", 0)),
        "final_vision_primary_accepted": bool(final_vision_primary_accepted),
        "final_vision_corroborated": bool(final_vision_corroborated),
        "final_vision_occlusion_aware": bool(final_vision_occlusion_aware),
        "final_tactile_hold_ok": bool(tactile_hold_ok),
        "final_low_confidence_vision_ok": bool(low_confidence_vision_ok),
        "final_occluded_vision_mask_ok": bool(
            final_vision.get("status") == "ok"
            and int(final_vision.get("mask_pixels", 0)) >= int(getattr(args, "pinch_final_occlusion_aware_min_mask_pixels", 120))
        ),
        "final_verification_mode": final_verification_mode,
        ...
        "final_vision_mask_retention_ratio": float(final_vision.get("mask_retention_ratio", 1.0)),
        "final_vision_visibility_fraction": float(final_vision.get("visibility_fraction", 0.0)),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Closed-loop eval for Stage3.10C safety ACT/CVAE chunk policy.")
    ...
    parser.add_argument(
        ...
        choices=["strict_vision", "vision_tactile_corroborated", "vision_tactile_occlusion_aware"],
        default="strict_vision",
    )
    ...
    parser.add_argument("--pinch-final-occlusion-aware-min-mask-pixels", type=int, default=120)
    ...
    parser.add_argument(
        ...
        default="none",
        help="Optional noisy virtual-camera scenario for initial pose acquisition, e.g. mask_dropout_30 or combined_hard.",
    )
    parser.add_argument(
        "--final-vision-stress-scenario",
        default="none",
        help="Optional noisy virtual-camera scenario for final pinch vision verification, e.g. synthetic_occluder_45.",
    )
    ...
    parser.add_argument("--policy-vision-nominal-min-confidence", type=float, default=0.95)
    parser.add_argument("--policy-vision-nominal-max-occlusion", type=float, default=0.05)
    parser.add_argument("--policy-vision-nominal-max-noise-std", type=float, default=0.0)
    parser.add_argument("--vision-stress-min-mask-pixels", type=int, default=80)
    parser.add_argument(
        "--safety-head-shadow-checkpoint",
        type=Path,
        default=None,
        help="Optional Stage3.10D-C safety-head checkpoint. When set, predictions are logged after each episode and never affect actions.",
    )
    parser.add_argument("--safety-head-shadow-threshold", type=float, default=0.5)
    parser.add_argument("--safety-head-shadow-case-id", default="online_shadow")
    ...
    safety_head_shadow = None
    if args.safety_head_shadow_checkpoint is not None:
        safety_head_shadow = load_safety_head_shadow(Path(args.safety_head_shadow_checkpoint))
    ...
    for skill in skills:
        ...
        for local_idx in range(int(args.episodes_per_skill)):
            ...
            if safety_head_shadow is not None:
                attach_safety_head_shadow(
                    row,
                    args=args,
                    head=safety_head_shadow,
                    threshold=float(args.safety_head_shadow_threshold),
                )
            results.append(row)
            s = row.get("summary", {})
            lift = s.get("final_lift_height_m", s.get("true_lift_height_m", float("nan")))
    ...
    payload = {
        ...
        "safety_head_shadow": summarize_safety_head_shadow(results),
    }
