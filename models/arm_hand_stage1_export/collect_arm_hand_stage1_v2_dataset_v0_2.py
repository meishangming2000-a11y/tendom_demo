#!/usr/bin/env python3
"""Collect Stage2 lift-ball dataset v0.2 with targeted right-edge recovery.

Dataset v0.2 keeps the v0.1 reset grid and adds a small recovery neighborhood
around the right-edge offsets that timed out in the v0.1 obs+phase rollout.
Rows keep `actions` as the applied behavior action for deterministic replay
and `expert_actions` as the BC training target.
"""
...
from collect_arm_hand_stage1_v2_dataset_v0_1 import (
    DEFAULT_X_OFFSETS,
    DEFAULT_Y_OFFSETS,
    DEFAULT_Z_OFFSETS,
    PROFILES,
    BehaviorProfile,
    make_behavior_action,
    parse_profiles,
    scaled_phase_args,
)
from run_arm_hand_stage1_v2_ball_pose_sweep import blend_targets, grid_offsets
...
DOCS = ROOT / "docs"
META = ROOT / "metadata"
DEFAULT_DATASET = DATA / "arm_hand_stage1_v2_lift_ball_dataset_v0_2.npz"
DEFAULT_REPORT = DOCS / "arm_hand_stage1_v2_dataset_v0_2_report.md"
DEFAULT_META = META / "arm_hand_stage1_v2_dataset_v0_2.json"

RECOVERY_EXTRA_OFFSETS = [
    [0.02, -0.015, 0.0],
    [0.02, -0.005, 0.0],
    [0.02, 0.015, 0.0],
    [0.02, 0.025, 0.0],
...
class RecoveryMode:
    name: str
    arm_delta: dict[str, float]
    reason: str


RECOVERY_MODES = [
    RecoveryMode("nominal", {}, "Use the original v0.1 scripted arm path."),
    RecoveryMode(
        "right_edge_mid_y",
        {"j1": 0.04, "j3": 0.0, "j4": -0.10},
        "Right-edge offsets with y below the upper boundary need a slightly shifted approach.",
    ),
    RecoveryMode(
        "right_edge_high_y",
        {"j1": 0.18, "j3": 0.05, "j4": -0.10},
        "Upper right-edge offsets need a stronger approach yaw/shape correction.",
    ),
    RecoveryMode(
        "right_edge_upper_y",
        {"j1": 0.18, "j2": -0.04, "j3": 0.05, "j4": -0.10},
        "The uppermost right-edge recovery offset needs a slightly deeper arm approach.",
    ),
]


def mode_by_name() -> dict[str, RecoveryMode]:
    return {mode.name: mode for mode in RECOVERY_MODES}


def select_recovery_mode(offset: np.ndarray) -> RecoveryMode:
    x, y, _ = [float(v) for v in offset]
    if x >= 0.018 and y >= 0.024:
        return mode_by_name()["right_edge_upper_y"]
    if x >= 0.018 and y >= 0.015:
        return mode_by_name()["right_edge_high_y"]
    if x >= 0.018 and y >= -0.015:
        return mode_by_name()["right_edge_mid_y"]
    return mode_by_name()["nominal"]


def unique_offsets(offsets: Iterable[np.ndarray]) -> list[np.ndarray]:
    seen: set[tuple[float, float, float]] = set()
    out: list[np.ndarray] = []
    for offset in offsets:
        arr = np.asarray(offset, dtype=np.float64)
        key = tuple(round(float(v), 6) for v in arr.tolist())
        if key in seen:
            continue
        seen.add(key)
        out.append(arr)
...
def build_v0_2_offsets(args) -> list[np.ndarray]:
    base = grid_offsets(args.x_offsets, args.y_offsets, args.z_offsets)
    extra = [np.asarray(item, dtype=np.float64) for item in RECOVERY_EXTRA_OFFSETS] if args.include_recovery_offsets else []
    return unique_offsets([*base, *extra])


def apply_arm_delta(target: dict[str, float], mode: RecoveryMode) -> dict[str, float]:
    out = dict(target)
    for joint, delta in mode.arm_delta.items():
        out[joint] = float(out.get(joint, 0.0)) + float(delta)
    return out


def build_recovery_phases(
    default_targets: dict[str, float],
    args,
    mode: RecoveryMode,
) -> list[tuple[str, dict[str, float], dict[str, float], int]]:
    pre_approach = apply_arm_delta({**default_targets, **lift_targets.ARM_PRE_APPROACH}, mode)
    approach = apply_arm_delta({**default_targets, **lift_targets.ARM_APPROACH}, mode)
    arm_lift = apply_arm_delta({**default_targets, **lift_targets.ARM_LIFT}, mode)
    ...
    lift = {**arm_lift, **lift_targets.LONG_FINGER_TARGETS, **lift_targets.THUMB_SMOKE_TARGETS}
    return [
...
def collect_episode(
    ...
    mode = select_recovery_mode(offset)
    ...
    phases = build_recovery_phases(default_targets, phase_args, mode)
    ...
    mode_id = [item.name for item in RECOVERY_MODES].index(mode.name)
    arm_delta = np.asarray(
        [mode.arm_delta.get("j1", 0.0), mode.arm_delta.get("j2", 0.0), mode.arm_delta.get("j3", 0.0), mode.arm_delta.get("j4", 0.0)],
        dtype=np.float64,
    )
    ...
    for phase_id, (phase_name, start_targets, end_targets, steps) in enumerate(phases):
        ...
        for local_step in range(max(1, int(steps))):
            ...
            rows["recovery_mode_ids"].append(mode_id)
            rows["arm_target_deltas"].append(arm_delta.copy())
...
def write_dataset(dataset_path: Path, episodes: list[dict[str, Any]], api: ArmHandStage1TaskAPI, contract: dict[str, Any], args) -> None:
    ...
    np.savez_compressed(
        ...
        recovery_mode_ids=concatenate_rows(episodes, "recovery_mode_ids", np.int32),
        arm_target_deltas=concatenate_rows(episodes, "arm_target_deltas", np.float64),
        ...
        recovery_mode_table=np.asarray([mode.name for mode in RECOVERY_MODES], dtype=str),
        recovery_mode_json=np.asarray([json.dumps(json_ready(mode.__dict__), ensure_ascii=False) for mode in RECOVERY_MODES], dtype=str),
        task_name=np.asarray([TASK_NAME], dtype=str),
        contract_version=np.asarray([TASK_CONTRACT_VERSION], dtype=str),
        dataset_version=np.asarray(["v0.2"], dtype=str),
...
def summarize_payload(dataset_path: Path, episodes: list[dict[str, Any]], api: ArmHandStage1TaskAPI, args) -> dict[str, Any]:
    ...
    for ep in episodes:
        ...
        mode_counts[ep["recovery_mode"].name] = mode_counts.get(ep["recovery_mode"].name, 0) + 1
    return {
        ...
        "recovery_mode_counts": mode_counts,
        "grid": {
            "x_offsets": args.x_offsets,
            "y_offsets": args.y_offsets,
            "z_offsets": args.z_offsets,
            "recovery_extra_offsets": RECOVERY_EXTRA_OFFSETS if args.include_recovery_offsets else [],
        },
        "profiles": [profile.__dict__ for profile in PROFILES if profile.name in args.profile_names],
        "recovery_modes": [mode.__dict__ for mode in RECOVERY_MODES],
        "episode_summaries": [
            {
                "episode_id": ep["episode_id"],
                "profile": ep["profile"].name,
                "recovery_mode": ep["recovery_mode"].name,
                "arm_delta": ep["arm_delta"],
        ...
        "notes": [
            "Dataset v0.2 targets the two right-edge timeout offsets found in the v0.1 obs+phase rollout.",
...
def write_report(payload: dict[str, Any], report_path: Path, meta_path: Path) -> None:
    ...
    lines = [
        ...
        f"- Recovery mode counts: `{payload['recovery_mode_counts']}`\n",
    ...
    for mode in payload["recovery_modes"]:
        lines.append(f"| {mode['name']} | `{mode['arm_delta']}` | {mode['reason']} |\n")
    lines.extend(
        [
            "\n## Episode Summary\n\n",
    ...
    for ep in payload["episode_summaries"]:
        ...
        lines.append(
            f"| {ep['episode_id']} | {ep['profile']} | {ep['recovery_mode']} | `{np.round(ep['offset'], 5).tolist()}` | "
            f"{ep['step_count']} | {ep['terminal_phase']} | {final['episode_status']} | {final['terminal_reason']} | "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Stage2 lift-ball dataset v0.2 with targeted recovery demos.")
    ...
    parser.add_argument("--include-recovery-offsets", action=argparse.BooleanOptionalAction, default=True)
    ...
    offsets = build_v0_2_offsets(args)
    ...
    print(f"Dataset v0.2 status: {payload['success_count']}/{payload['episodes']} success")
