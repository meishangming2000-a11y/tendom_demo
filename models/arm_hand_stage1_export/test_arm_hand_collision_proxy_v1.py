#!/usr/bin/env python3
"""Test collision-proxy v1 with free-ball and scripted-close smoke checks."""
...
DEFAULT_SCENE = ROOT / "mjcf" / "scene_arm_hand_export4_collision_proxy_v1_ball.xml"
DOCS = ROOT / "docs"
META = ROOT / "metadata"
VIS = DOCS / "visual_checks_arm_hand_collision_proxy_v1"
REPORT = DOCS / "arm_hand_collision_proxy_v1_smoke_report.md"
META_OUT = META / "arm_hand_collision_proxy_v1_smoke.json"
...
def run_free_ball_open(model, data, mujoco, steps: int) -> dict[str, Any]:
    ...
    screenshots = {"start": render(model, data, mujoco, VIS / "v1_free_ball_start.png")}
    ...
    screenshots["end"] = render(model, data, mujoco, VIS / "v1_free_ball_after_open.png")
    return {
        "finite": finite,
        "steps": steps,
        "start_ball": start,
...
def run_scripted_pinned(model, data, mujoco, steps: int) -> dict[str, Any]:
    ...
    for phase_idx, (phase, targets) in enumerate(PHASES):
        ...
        screenshots[phase] = render(model, data, mujoco, VIS / f"v1_scripted_{phase}.png")
