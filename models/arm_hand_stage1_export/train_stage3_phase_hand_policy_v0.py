#!/usr/bin/env python3
"""Train Stage3.6 phase-specific learned hand policy.

Input:  Stage3 sensor abstraction `obs`
Target: hand/finger slice of `expert_actions`

The arm and wrist remain scripted. This policy is intentionally scoped to
contact/lift/hold phases so it does not relearn the high-sensitivity approach.
"""
...
DEFAULT_REPORT = DOCS / "stage3_phase_hand_policy_v0_train_report.md"
DEFAULT_METADATA = META / "stage3_phase_hand_policy_v0_train.json"

STATUS = "stage3_phase_hand_policy_v0_experimental_not_promoted"
DEFAULT_LEARNED_PHASES = (
...
def phase_mask_from_names(data, learned_phases: list[str]) -> np.ndarray:
    phase_names = string_list(data["expert_phase_table"])
    missing = [name for name in learned_phases if name not in phase_names]
    if missing:
        raise ValueError(f"Unknown learned phases {missing}; available={phase_names}")
    allowed = {phase_names.index(name) for name in learned_phases}
    phase_ids = data["phase_ids"].astype(np.int32)
    return np.asarray([int(phase_id) in allowed for phase_id in phase_ids], dtype=bool)


def hand_indices_from_dataset(data, hand_start_index: int) -> np.ndarray:
    actuator_names = string_list(data["actuator_names"])
    if hand_start_index < 0 or hand_start_index >= len(actuator_names):
        raise ValueError(f"hand_start_index={hand_start_index} outside actuator count={len(actuator_names)}")
    return np.arange(int(hand_start_index), len(actuator_names), dtype=np.int32)


def feature_config_from_dataset(data, feature_mode: str, learned_phases: list[str], hand_indices: np.ndarray) -> dict[str, Any]:
    if feature_mode != "obs_only":
        raise ValueError("Stage3 phase hand policy currently supports only feature_mode=obs_only.")
    actuator_names = string_list(data["actuator_names"])
    return {
        ...
        "expert_phase_names": string_list(data["expert_phase_table"]),
        "learned_phases": list(learned_phases),
        "hand_action_indices": [int(i) for i in hand_indices.tolist()],
        "hand_action_names": [actuator_names[int(i)] for i in hand_indices.tolist()],
        "arm_wrist_policy": "scripted_expert",
    }


def phase_counts(data, mask: np.ndarray) -> dict[str, int]:
    phase_names = string_list(data["expert_phase_table"])
    phase_ids = data["phase_ids"].astype(np.int32)
    return {name: int(np.sum(mask & (phase_ids == idx))) for idx, name in enumerate(phase_names)}
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Hand action dim: `{payload['act_dim']}`\n",
        f"- Learned phases: `{payload['learned_phases']}`\n",
        f"- Arm/wrist policy: `scripted_expert`\n",
        ...
        f"- Train hand-action RMSE raw: `{payload['train_action_rmse_raw']:.8f}`\n",
        f"- Val hand-action RMSE raw: `{payload['val_action_rmse_raw']:.8f}`\n",
        "- Training ready: **No, closed-loop MuJoCo eval required**\n\n",
        "## Phase Samples Used\n\n",
        "| phase | selected rows |\n",
        "|---|---:|\n",
    ]
    for phase, count in payload["selected_phase_counts"].items():
    ...
    for name, value in zip(payload["hand_action_names"], payload["val_action_rmse_by_dim"]):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train Stage3.6 phase-specific hand policy.")
    ...
    parser.add_argument("--learned-phases", default=",".join(DEFAULT_LEARNED_PHASES))
    ...
    require_fields(data, ["obs", args.action_field, "episode_ids", "phase_ids", "expert_phase_table", "actuator_names", "dataset_version", "contract_version"])
    learned_phases = parse_csv(str(args.learned_phases))
    selected_phase_mask = phase_mask_from_names(data, learned_phases)
    if not np.any(selected_phase_mask):
        raise RuntimeError("No samples selected for learned phases.")
    hand_indices = hand_indices_from_dataset(data, int(args.hand_start_index))
    feature_config = feature_config_from_dataset(data, args.feature_mode, learned_phases, hand_indices)

    features = data["obs"].astype(np.float32)
    full_actions = data[args.action_field].astype(np.float32)
    actions = full_actions[:, hand_indices].astype(np.float32)
    episode_ids = data["episode_ids"].astype(np.int32)

    if bool(args.train_all):
        train_mask = selected_phase_mask.copy()
        val_mask = selected_phase_mask.copy()
        train_episodes = sorted(int(v) for v in np.unique(episode_ids[selected_phase_mask]))
        val_episodes = train_episodes.copy()
    else:
        split_train, split_val, train_episodes, val_episodes = split_by_episode(episode_ids, args.val_fraction, args.seed)
        train_mask = selected_phase_mask & split_train
        val_mask = selected_phase_mask & split_val
    ...
    selected_phase_counts = phase_counts(data, selected_phase_mask)
    ...
    checkpoint = {
        ...
        "base_obs_dim": int(data["obs"].shape[1]),
        "act_dim": int(actions.shape[1]),
        "full_act_dim": int(full_actions.shape[1]),
        "hidden_dim": int(args.hidden_dim),
        "depth": int(args.depth),
        ...
        "full_action_names": string_list(data["actuator_names"]),
        "hand_action_indices": [int(i) for i in hand_indices.tolist()],
        "hand_action_names": feature_config["hand_action_names"],
